...
### Anti-Deletion Keepalive
A scheduled CPU load prevents Oracle from reclaiming the instance due to idleness.

*   **Schedule:** Every 2 hours
*   **Action:** 2 Cores for 300 seconds

```bash
(crontab -l 2>/dev/null; echo "0 */6 * * * /usr/bin/stress-ng --cpu 2 --timeout 300s") | crontab -
```
...
4.  **DNS Override:** Set Global Nameserver to `100.108.211.122` (Self) to enforce AdGuard blocking.
...
| Service | URL | Network Scope |
| :--- | :--- | :--- |
| **Nginx Proxy Manager** | `http://100.108.211.122:81` | 🔒 Private (VPN Only) |
| **AdGuard Home** | `http://100.108.211.122:8083` | 🔒 Private (VPN Only) |
| **Status Page** | `https://status.snawf.my.id` | 🌍 Public Internet |
| **Syncthing** | `http://100.108.211.122:8384` | 🔒 Private (VPN Only) |
